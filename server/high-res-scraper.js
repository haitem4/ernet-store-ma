...
const getSearchQuery = (p) => {
    ...
    if (p.brand === 'BLUEMEGA' && p.name.includes('Toner')) {
        return `HP Toner Cartridge IT hardware ${cleanName.substring(0, 30)}`;
    }
    ...
    return `${p.brand} ${cleanName} ${p.sku}`.substring(0, 80);
};
...
(async () => {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        console.log(`[${i+1}/${items.length}] Searching High-Res for: ${query}`);
            ...
            if (urls && urls.length > 0) {
                ...
                for (let j = 0; j < Math.min(5, urls.length); j++) {
                    try {
                        ...
                        if (stats.size > 8000) { // must be larger than 8KB to be a decent high res image
                            console.log(` => Downloaded HIGH RES image for ${p.sku} from ${urls[j]}`);
                            downloaded = true;
                            break;
                        }
                    } catch (e) { }
    ...
    await browser.close();
    console.log("High Res Download Done!");
})();
